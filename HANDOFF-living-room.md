...
- **A hovered volume comes forward.** `bookshelf.pulls(listener)` reports which spine is being reached for; the scene eases that jacket 20mm along +Z and back, snapping instead under reduced motion. Only the printed jacket moves: the bake joins the printed planes for one bake and one draw, and `scene.ts` slices them apart again against `book-slots.json`, so 55 two-triangle meshes share the one atlas material. The leather body stays in the row and covers the gap. Head-on this reads as the volume easing out; off to the side of the frame it also reveals a dark strip of the body beside the jacket, which reads as its own shadow. `PULL` is the one constant to turn if that ever wants to be smaller.

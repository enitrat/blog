...
- The user said "Dont run tests, dont visually verify at first: just implement the scene properly." Earlier instructions also prohibited writing tests. No test writing or execution is authorized. Browser inspection, screenshots, and poster regeneration have remained deferred throughout these implementation passes. Revisit visual verification when the user requests that phase.
...
- **Visual refinement is pending.** The hierarchy is agreed and implemented. Camera composition, transitions, dot placement, phone framing, overlap, and actual text readability await a user-requested visual pass. The last patch only centered the row dots and changed hover cursors; it did not move the original overview hotspot.
...
- **Library changes require appropriate asset regeneration.** ISBN coverage checks do not certify that changed titles, page counts, or other metadata match old artwork and geometry. The generator still limits printed title lines and compresses author lettering. Treat typography refinements as source-artwork work, not something a camera adjustment can repair.
- **The poster is stale relative to the new print atlas.** The current file includes an earlier Opus change; this session did not recapture it. Keep the opaque renderer and hide all overlaid controls in a future poster capture.
